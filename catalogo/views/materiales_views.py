...
def materiales_lista(request):
    ...
    filter_fields = [
        {"name": "id", "type": "text", "placeholder": "ID", "value": material_id},
        {"name": "material", "type": "text", "placeholder": "Material", "value": material},
        {
            "name": "tipo",
            "type": "select",
            ...
            "value": tipo,
            "options": [
                {"value": t.codigo_catalogo, "label": t.nombre_empresa, "selected": t.codigo_catalogo == tipo}
...
def crear_material(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                registrar_actividad(
                    request, "crear", "catalogo", material.pk,
                ...
                return redirect("catalogo:materiales_lista")
    ...
    return render(request, "catalogo/form.html", context)
...
def editar_material(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            registrar_actividad(
                request, "editar", "catalogo", material.pk, f"Material editado: {material.nombre}"
            )
...
def eliminar_material(request, id):
    ...
    material.delete()
    registrar_actividad(request, "eliminar", "catalogo", id, f"Material eliminado: {nombre}")
...
def cambiar_estado_material(request, id):
    ...
    registrar_actividad(request, "actualizar", "catalogo", material.pk, f"Material {estado}: {material.nombre}")
...
def editar_stock(request, id):
    ...
    if request.method == "POST":
        try:
            ...
            return redirect("catalogo:stock_lista")
    ...
    return render(request, "catalogo/form_stock.html", context)
