...
def obtener_contexto_datos(force_refresh=False, usuario=None):
    # Primero, inicializamos diccionario vacío
    ...
    es_admin_o_empleado = rol_usuario in ["admin", "administrador", "empleado"]

    # Si es admin, empleado o no hay usuario (anónimo, pero views bloquean anónimo), obtenemos datos globales
    if es_admin_o_empleado or not usuario:
        ...
        try:
            ...
            asignaciones_activas = ConductorVehiculo.objects.filter(
                fecha_fin__isnull=True
            ).select_related("conductor__usuario", "vehiculo")
            ...
            data.update(
                {
                    ...
                    "total_stock": Stock.objects.aggregate(total=Sum("cantidad_actual"))["total"]
                    or 0,
            ...
            # Cachear solo datos globales (sin usuario)
            if not usuario:
                cache.set(CONTEXT_CACHE_KEY, data, CONTEXT_CACHE_TTL)

        except Exception:
            logger.exception("Error obteniendo contexto de datos globales")
        ...
        if rol_usuario == "cliente":
            try:
                ...
                if cliente_obj:
                    ...
                    data["mis_pedidos_pendientes"] = pedidos_cliente.filter(
                        estado="pendiente"
                    ).count()
                    data["mis_pedidos_entregados"] = pedidos_cliente.filter(
                        estado="entregado"
                    ).count()
                ...
                if conductor_obj:
                    ...
                    asignacion = (
                        ConductorVehiculo.objects.filter(
                            conductor=conductor_obj, fecha_fin__isnull=True
                        )
                    ...
                    if asignacion:
                        data["mi_vehiculo"] = (
                            f"{asignacion.vehiculo.marca} {asignacion.vehiculo.modelo} (Placa: {asignacion.vehiculo.placa})"
                        )
                    else:
