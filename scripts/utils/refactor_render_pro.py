        ...
        try:
            ...
            if context_idx < len(new_args):
                ...
                new_node = ast.Call(
                    func=node.func,
                    args=new_args,
                    keywords=[kw for kw in node.keywords if kw.arg != "context"],
                )
            else:
                # Era un kwarg
                new_keywords = []
                for kw in node.keywords:
                    if kw.arg == "context":
                        new_keywords.append(
                            ast.keyword(
                                arg="context", value=ast.Name(id=ctx_var_name, ctx=ast.Load())
                            )
                        )
                    else:
            ...
            if verbose:
                print(
                    color_text(
                        f"    [SKIP] No se pudo reconstruir call en línea {render_info['line']}",
                        "yellow",
                    )
                )
            continue
...
def generate_report(
    ...
    total_dict_renders = sum(
        len(r["renders_with_dict"]) + len(r["renders_with_dict_call"]) for r in all_results
    )
    ...
    with open(report_path_obj, "w", encoding="utf-8") as f:
        if ext == ".json":
            ...
            f.write(
                f"| Renders problemáticos (dict literal / dict call) | {total_dict_renders} |\n"
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--backup-dir", type=str, help="Directorio para backups (default: <base_dir>/backups)"
    )
    ...
    parser.add_argument(
        "--check-templates", action="store_true", help="Verifica existencia de templates"
    )
    parser.add_argument(
        "--base-dir", type=str, help="Directorio raíz del proyecto (default: directorio actual)"
    )
    ...
    if args.base_dir:
        ...
        if not base_dir.is_dir():
            print(
                color_text(f"[ERROR] --base-dir no existe o no es un directorio: {base_dir}", "red")
            )
            return 2
    else:
        base_dir = Path.cwd()
    ...
    for file_path in view_files:
        ...
        if not args.quiet:
            if results["has_errors"]:
                ...
                if args.verbose:
                    print(
                        color_text(f"  [SKIP]    {rel_str}: sin imports de render Django", "white")
                    )
                ...
                print(color_text(f"  [ALERTA]  {rel_str}: {', '.join(msg_parts)}", "yellow"))
                if args.verbose:
                    for item in results["renders_with_dict"]:
                        print(
                            color_text(
                                f"      dict literal → línea {item['line']}: {item.get('template', '?')}",
                                "yellow",
                            )
                        )
                    for item in results["renders_with_dict_call"]:
                        print(
                            color_text(
                                f"      dict() call  → línea {item['line']}: {item.get('template', '?')}",
                                "yellow",
                            )
                        )
            else:
                print(
                    color_text(
                        f"  [OK]      {rel_str}: {len(results['renders_ok'])} render(s) correctos",
                        "green",
                    )
                )
    ...
    if not args.quiet:
        ...
        if args.fix:
            print(
                f"  {'Correcciones aplicadas':<35}: {color_text(str(total_corrections), 'green')}"
            )
        if total_errors > 0:
        ...
        if total_problematic == 0 and total_errors == 0:
            print(
                color_text(
                    "  ✅ ¡Sin alertas! Todos los renders usan variables de contexto.",
                    "green",
                    "bright",
                )
            )
