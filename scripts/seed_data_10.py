...
from apps.usuarios.models import (
    Usuario, EPS, Conductor, Vehiculo, ConductorVehiculo, 
    Proveedor, MaterialConstruccion, Stock, MetodoPago, Notificacion,
    Catalogo
)
...
def seed_database_instance(db_alias):
    ...
    # ==========================================
    eps_list = []
    eps_nombres = ['Sura', 'Sanitas', 'Compensar', 'Salud Total', 'Famisanar', 'Coosalud', 'Nueva EPS', 'Aliansalud', 'Mutual Ser', 'Capresoca']
    ...
    # ==========================================
    vehiculos = []
    marcas = ['Toyota', 'Chevrolet', 'Ford', 'Hino', 'Kenworth', 'JAC', 'Foton', 'Volvo', 'Scania', 'Mercedes']
    ...
    for idx in range(10):
        v = Vehiculo.objects.using(db_alias).create(
            ...
            marca=marcas[idx],
            modelo=modelos[idx],
            tipo_vehiculo='Volqueta' if idx % 3 == 0 else ('Camión Estacas' if idx % 3 == 1 else 'Tractomula'),
    ...
    # ==========================================
    proveedores = []
    prov_nombres = ['Aceros Bogotá', 'Cales y Arenas SAS', 'Ladrillera Central', 'Cementos Argos S.A.', 'Hierros Occidente', 'Tubos del Caribe', 'Herramientas Pro', 'Agregados del Norte', 'Ladrillera del Sur', 'Pinturas Global']
    ...
    for cod, nom in tipos_cat:
        cat = Catalogo.objects.using(db_alias).create(
            codigo_catalogo=cod,
            nombre_empresa=nom
        )
    ...
    for cod, met in metodos_base:
        mp = MetodoPago.objects.using(db_alias).create(
    ...
    for idx, ped in enumerate(pedidos):
        ...
        DetallePedido.objects.using(db_alias).create(
            ...
            material=mat_temp,
            cantidad=10 + idx,
            precio_unitario=mat_temp.precio
        )
        ped.calcular_total()
    ...
    for idx, ped in enumerate(pedidos):
        Entrega.objects.using(db_alias).create(
            ...
            estado='pendiente' if ped.estado == 'pendiente' else ('en_ruta' if ped.estado == 'en_ruta' else 'entregado'),
            fecha_salida=timezone.now() if ped.estado != 'pendiente' else None,
            fecha_entrega=timezone.now() + timedelta(hours=3) if ped.estado == 'entregado' else None
        )
    ...
    print(f"  [ÉXITO] ¡Todas las 22 tablas en la base de datos [{db_alias}] fueron pobladas exitosamente!")
...
def run_seed():
    ...
    print("  -> ADMINISTRADOR:  Correo: admin@c.com  / Contraseña: @dmin123 (Documento: 0000000000)")
    print("  -> CLIENTE:        Correo: client@c.com / Contraseña: @dmin123 (Documento: 0000000001)")
    print("=======================================================")
