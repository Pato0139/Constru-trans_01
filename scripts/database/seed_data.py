...
def setup_data():
    ...
    try:
        admin_user, created = User.objects.get_or_create(
            username="Edward_Fonseca", defaults={"email": "edwardf5432@gmail.com"}
        )
        if created:
        ...
        catalogo, _ = Catalogo.objects.get_or_create(
            codigo_catalogo="CAT001", defaults={"nombre_empresa": "Constru-Trans"}
        )
        ...
        try:
            metodo_pago, _ = MetodoPago.objects.get_or_create(
                codigo_metodo_pago="EFECTIVO", defaults={"metodo": "Efectivo"}
            )
        except Exception:
            ...
            try:
                ...
                metodo_pago = MetodoPago.objects.create(
                    codigo_metodo_pago="EFECTIVO", metodo="Efectivo"
                )
        ...
        materiales_data = [
            ...
            },
            {
                ...
                "precio": 1200,
                "descripcion": "Ladrillo de arcilla cocida",
                "unidad_medida": "und",
            },
        ]
        ...
        for m_data in materiales_data:
            ...
            stock, s_created = Stock.objects.get_or_create(
                material=mat, defaults={"cantidad_actual": 1000, "ubicacion": "Bodega Principal"}
            )
            if not s_created:
        ...
        conductores_data = [
            {
                "username": "carlos_chofer",
                "email": "carlos@constru.com",
                "nombres": "Carlos",
                "apellidos": "Mendoza",
                "doc": "778899",
                "placa": "TRX-101",
                "marca": "Toyota",
                "modelo": "Hilux",
                "tipo_vehiculo": "Camión 5 Ton",
                "capacidad_carga": 5.0,
            },
            {
                "username": "pedro_trans",
                "email": "pedro@constru.com",
                "nombres": "Pedro",
                "apellidos": "Salas",
                "doc": "554433",
                "placa": "KLM-202",
                "marca": "Chevrolet",
                "modelo": "Silverado",
                "tipo_vehiculo": "Bolqueta 10 Ton",
                "capacidad_carga": 10.0,
            },
            {
                "username": "luis_driver",
                "email": "luis@constru.com",
                "nombres": "Luis",
                "apellidos": "García",
                "doc": "998877",
                "placa": "ABC-123",
        ...
        for c_data in conductores_data:
            u_cond, created = User.objects.get_or_create(
                username=c_data["username"], defaults={"email": c_data["email"]}
            )
            if created:
            ...
            if cond_created or v_created:
                ConductorVehiculo.objects.get_or_create(conductor=cond_profile, vehiculo=vehiculo)
        ...
        clientes_data = [
            {
                "username": "constructora_alfa",
                "email": "proyectos@alfa.com",
                "nombres": "Ing. Roberto",
                "apellidos": "Torres",
                "doc": "112233",
                "empresa": "Constructora Alfa SAS",
            },
            {
            ...
            {
                "username": "obra_norte",
                "email": "obra@norte.com",
                "nombres": "Andrés",
                "apellidos": "Rodríguez",
                "doc": "776655",
                "empresa": "Obra Norte Ltda",
            },
        ]

        for cl_data in clientes_data:
            u_cl, created = User.objects.get_or_create(
                username=cl_data["username"], defaults={"email": cl_data["email"]}
            )
            if created:
            ...
            for idx, estado_p in enumerate(estados_pedido):
                ...
                print(
                    f"Cliente {cl_data['empresa']} - Pedido #{pedido.codigo_pedido} ({estado_p}) - Factura #{factura.numero} creado."
                )
