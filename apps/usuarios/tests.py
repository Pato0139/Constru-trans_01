...
from .models import Conductor, ConductorVehiculo, Material, Stock, UnidadMedida, Usuario, Vehiculo
...
class ConstruTransTestSuite(TestCase):
    def setUp(self):
        ...
        self.unidad_m3 = UnidadMedida.objects.create(codigo="M3", nombre="m3", abreviatura="m3")
    ...
    def test_05_asignar_vehiculo_a_conductor(self):
        ...
        primera_asignacion = ConductorVehiculo.objects.filter(
            conductor=self.conductor_profile
        ).first()
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            ConductorVehiculo.objects.filter(conductor=self.conductor_profile).count(), 2
        )

        asignaciones = ConductorVehiculo.objects.filter(conductor=self.conductor_profile).order_by(
            "-fecha_asignacion"
        )
