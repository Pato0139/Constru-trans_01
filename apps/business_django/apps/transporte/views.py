...
from apps.usuarios.models import Conductor, ConductorVehiculo, Vehiculo
...
def crear_vehiculo(request):
    ...
    conductores = Conductor.objects.select_related("usuario").order_by(
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if conductor_id:
                ...
                # Desasignar de cualquier vehículo anterior
                ConductorVehiculo.objects.filter(
                    conductor=conductor_perfil, fecha_fin__isnull=True
            ...
            messages.error(request, "Error: La placa ya existe.")
        except Exception as e:
            messages.error(request, f"Error: {str(e)}")

    context = {"action": "crear", "conductores": conductores}
...
def editar_vehiculo(request, id):
    ...
    if request.method == "POST":
        ...
        if nuevo_estado == "disponible" and entregas_activas:
            messages.error(
                request,
                "No se puede marcar como disponible porque tiene entregas pendientes o en curso.",
            )
        else:
            ...
            conductor_id = request.POST.get("conductor")
            try:
                ConductorVehiculo.objects.filter(vehiculo=vehiculo, fecha_fin__isnull=True).update(
                    fecha_fin=now()
                )
                ...
                if conductor_id:
                    ...
                    ConductorVehiculo.objects.filter(
                        conductor=conductor_perfil, fecha_fin__isnull=True
                    ).exclude(vehiculo=vehiculo).update(fecha_fin=now())
                    ConductorVehiculo.objects.create(conductor=conductor_perfil, vehiculo=vehiculo)
