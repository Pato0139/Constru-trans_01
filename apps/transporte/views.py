...
def crear_vehiculo(request):
    ...
    conductores_disponibles = (
        ...
        .exclude(perfil_conductor__asignaciones_vehiculo__fecha_fin__isnull=False)
        .exclude(
            id__in=ConductorVehiculo.objects.filter(fecha_fin__isnull=True).values(
                "conductor__usuario"
            )
        )
        .distinct()
    ...
    return render(
        request, "transporte/form.html", {"action": "crear", "conductores": conductores_disponibles}
    )
...
def editar_vehiculo(request, id):
    ...
    if conductor_actual:
        ...
        conductores = (
            conductores_disponibles | Usuario.objects.filter(id=conductor_actual.id)
        ).distinct()
    else:
    ...
    return render(
        request,
        "transporte/form.html",
        {"vehiculo": vehiculo, "action": "editar", "conductores": conductores},
    )
